...
保存到 `~/.pi/learnings/<project>/entries/*.md`：
...
1. **加载已有学习** — 先读取 `~/.pi/learnings/<project>/entries/` 中的已有条目
...
本 skill 是复利学习的**唯一入口和真相源**。三个组件分工明确，不重复：

| 组件 | 角色 | 职责 |
|------|------|------|
| `continuous-learning`（本 skill）| 入口/标准 | `/learn` 流程编排、提取标准、置信度阈值、升级规则 |
| `learn-agent`（子代理）| 执行者 | 由本 skill 派发，分析会话并输出 YAML 条目；也可直接调用 |
| `learning-persistence`（plugin）| 存储 | 项目检测、条目读写、置信度进化、会话启动加载高置信度提醒 |

**纪律**：提取标准只在 skill 里定义一份；learn-agent 只执行不重复标准；存储逻辑只在 plugin 里。
